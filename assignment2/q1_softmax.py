...
def softmax(x):
    ...
    x -= tf.reduce_max(x, axis=1)[..., tf.newaxis]

    # Softmax calculation vectorised.
    out = tf.exp(x)
    out /= tf.reduce_sum(out, axis=1)[..., tf.newaxis]
...
def cross_entropy_loss(y, yhat):
    ...
    y = tf.to_float(y)  # Tensorflow works with floats.
    out = -tf.reduce_sum(y*tf.log(yhat), axis=1)  # Sum across each class.
    out = tf.reduce_sum(out)  # Sum the batch.
